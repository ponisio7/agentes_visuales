...
class PromptABEvaluator:
    ...
    # ------------------------------------------------------------
    # Consulta auxiliar para el builder
    # ------------------------------------------------------------
    @staticmethod
    def consultar_versiones(
        db_path: str, firma: str
    ) -> Dict[str, Optional[Dict]]:
        """
        Devuelve las versiones vigentes (activo y candidato) para una
        firma. Si no hay, los valores son None.

        Devuelve: {
            "activo":    {"id": int, "prompt": str} | None,
            "candidato": {"id": int, "prompt": str} | None,
        }
        """
        resultado = {"activo": None, "candidato": None}
        if not firma:
            return resultado
        try:
            with sqlite3.connect(db_path, timeout=5) as conn:
                conn.row_factory = sqlite3.Row
                for estado in ("activo", "candidato"):
                    row = conn.execute(
                        """SELECT id, prompt_nuevo FROM prompts_reescritos
                           WHERE firma = ? AND estado = ?
                           ORDER BY fecha DESC LIMIT 1""",
                        (firma, estado),
                    ).fetchone()
                    if row:
                        resultado[estado] = {
                            "id": row["id"],
                            "prompt": row["prompt_nuevo"],
                        }
        except Exception as e:
            logger.debug(f"consultar_versiones falló: {e}")
        return resultado