...
def registrar_ejecucion_en_aprendizaje(scheduler, db, plan, problema: str, duracion_total: float) -> Optional[int]:
        ...
        try:
            ...
            if plan_snap is not None and problema_snap:
                try:
                    ...
                    engine.registrar_resultado_plan(ejecucion_id=ejecucion_id, plan=plan_snap, tarea=problema_snap, resultado_texto=resultado_plan)
                except Exception as e:
    ...
    threading.Thread(target=_worker, name=f"learning-{ejecucion_id}", daemon=True).start()
    return ejecucion_id
