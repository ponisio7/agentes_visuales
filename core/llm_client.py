...
class LLMClient:
    ...
    def openai_client(self):
        """
        Acceso al cliente OpenAI subyacente (compartido, thread-safe).

        Usar SIEMPRE esta property desde código que ejecuta en workers
        concurrentes (executors.py::_ejecutar_llm). Crear un OpenAI() nuevo
        por llamada crea un httpx.Client + SSLContext nuevos, lo cual es un
        patrón conocido de corrupción de heap intermitente.
        """
        return self._client
...
# ============================================================
# CLIENTE COMPARTIDO (SINGLETON)
# ============================================================
#
# ✅ IMPORTANTE: usar SIEMPRE obtener_llm_client_compartido() en vez de
# LLMClient() directo en código que se ejecuta de forma repetida (diálogos,
# hilos de aprendizaje, ejecución de agentes). Crear un LLMClient nuevo crea
# también un OpenAI(...) nuevo por debajo, que a su vez crea un
# httpx.Client nuevo con su propio pool de conexiones y contexto SSL.
#
# Instanciar esto una y otra vez desde varios hilos a la vez (diálogo de
# resolución de problemas, hilo daemon de aprendizaje, ejecutores de
# agentes en paralelo) es un patrón conocido de corrupción de heap
# intermitente en procesos de larga duración. httpx.Client está
# documentado como seguro para compartir entre hilos, y la propia
# documentación del SDK de OpenAI recomienda crear un único cliente y
# reutilizarlo, en vez de crear uno por petición.
...
def obtener_llm_client_compartido(
    ...
    **kwargs: Any
    ...
    """
    Devuelve un único LLMClient compartido para todo el proceso,
    creándolo de forma perezosa (y thread-safe) la primera vez que
    se pide. Llamadas posteriores devuelven siempre la misma instancia,
    evitando crear un OpenAI/httpx.Client nuevo cada vez.
    """
